...
class FFT():
    ...
    def fit_1d(self, a: list[complex]) -> list[complex]:
        """
        ...
        y_even = self.fit_1d(a_even)
        y_odd = self.fit_1d(a_odd)
    ...
    def fit_inverse_1d(self, a: list[complex]) -> list[complex]:
        """
        Computes the Inverse Fast Fourier Transform (iFFT) of a sequence.

        Args:
            a (list[complex]): Input sequence in the frequency domain.

        Returns:
            list[complex]: Reconstructed time-domain sequence.
        """
        n = len(a)
        a_conj = [x.conjugate() for x in a]
        y = self.fit_1d(a_conj)
